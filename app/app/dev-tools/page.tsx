...
export default function DevToolsPage() {
  ...
  return (
    <div className="p-6 space-y-6 italic text-left">
      <h1 className="text-3xl font-black tracking-tighter">Dev Tools</h1>
      
      <div className="bg-[#FFD93D] border-2 border-border-black p-4 flex items-center gap-3 italic">
      ...
      {isConnected ? (
        <div className="space-y-6">
          <div className="neo-card bg-white space-y-2 italic text-left">
            <p className="text-[10px] font-black uppercase opacity-40 italic text-left">Connected Target</p>
            <p className="text-xs font-black break-all text-left italic">{address}</p>
          </div>
          ...
          {status && (
              <motion.div 
                ...
                className="bg-white border-2 border-border-black p-3 rounded-xl text-[10px] font-black uppercase italic"
              >
                  {status}
              </motion.div>
          )}
        ...
        <div className="neo-card bg-white p-8 text-center italic">
            <p className="text-sm font-black uppercase opacity-40 italic">Connect wallet to use dev tools</p>
        </div>
      )}
    </div>
  );
...
function BalanceBox({ label, value }: { label: string, value: string }) {
    return (
        <div className="bg-zinc-100 border-2 border-border-black p-3 italic text-left">
            <p className="text-[8px] font-black uppercase opacity-40 text-left italic">{label}</p>
            <p className="text-sm font-black italic text-left italic">{value}</p>
        </div>
    );
}
...
function DevButton({ label, icon: Icon, loading, onClick }: any) {
    return (
        <button 
            onClick={onClick}
            disabled={loading}
            className="neo-button w-full h-14 bg-white text-xs normal-case gap-3 italic"
        >
